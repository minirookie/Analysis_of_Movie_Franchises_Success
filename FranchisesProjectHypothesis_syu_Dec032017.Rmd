---
title: "FranchisesProjectHypothesis_syu_Dec032017"
author: "Shanshan Yu"
...
rm(list = ls())
setwd("/Users/syu/Documents_syu/")

#read two data files and order datasets in an invert chronical order
...
movie.gross[is.na(movie.gross)]# check how many records will be removed
...
unique(movie.meta.clean$color) # color variable has unnessary space
...
#########################################################################
# addjust the inflation ratio with CPI data
#########################################################################
...
# year inforamtion is treated as numeric here to ease the inflation adjustment
new.movie.year = as.numeric(movie.meta.clean$movie.year)

range(new.movie.year) # this dataset almost has a century of movie information
...
str(cpi.infla) # check the data type to match the data from two other datasets
...
# add back the inflation adjusted gross back to cleaned dataset
...
# add cleaned title and names back to the dataframes loaded from csv files
...
# check wether worng movies is removed
...
# compare these two subsetts length and visualize the comparison
movieNo.compare = c(length(row.names(fran.movie)),
  length(row.names(other.movie)), length(row.names(other.movie.nogross)))

# create image folder to store plots generated
image.folder = "/Users/syu/Documents_syu_Backup/Projects_Github/ImagesOfHypothesis/"

png(paste(image.folder, "barplot_MovieNumberComparison.png", sep = ""))
par(mar=c(22, 1, 3, 1))
...
png(paste(image.folder, "barplot_ProfitSumComparison.png", sep = ""))
par(mar=c(24, 1, 3, 1))
...
png(paste(image.folder, "barplot_AvgfitComparison.png", sep = ""))
par(mar=c(24, 1, 3, 1))
...
# Check the datatype of the franchis finace data
...
which(is.na(movfran.fina[, -c(2,7)]) == T, arr.ind = T) # all NA in No..of.Years 
...
# This means these movie franchies were in theater for a year
...
# the in-theater year info for frachises of one movie is probably left censored
...
movfran.fina[franyear.nv[franyear.left],] # To make sure index vectors are right
...
# remove the text data out of franchise finacial data for k-means clustering
...
png(paste(image.folder, "plot_KmeansElbow.png", sep = ""))
par(mar=c(12, 4, 4, 1))
...
# add the clusters to the movie franchise finacial data
...
png(paste(image.folder, "pie_K-clusterComposition.png", sep = ""))
par(mar = c(14,1,1,1))
...
movfran.result[movfran.result$K.cluster == 4,]$fran.title.clean
movfran.result[movfran.result$K.cluster == 1,]$fran.title.clean

movfran.result[movfran.result$K.cluster == 3, -1]
movfran.result[movfran.result$K.cluster == 4, -1]
movfran.result[movfran.result$K.cluster == 1, -1]


# Creat side by side visualization for comparison
...
png(paste(image.folder, "ggplot_KmeansAnalysis.png", sep = ""))
...
png(paste(image.folder, "ggplot_FranchiseGlobalBox.png", sep = ""))
...
## Check and remove NA values left the cleaned movie meta dataset
...
# further subset the cleaned dataset for regression
...
# change the matrixs to dataframe and combine two data frames
...
# use numeric data +/- chr data of color, language, country, content rating, year
...
# regression modeling
library("stats")
#create length variables for training datasets
set.seed(3456)
train.length.01 = floor(0.7*dim(movie.regr.01)[1])
train.length.02 = floor(0.7*dim(movie.regr.02)[1])

# generate random index with sample function to create training and test datasets
# exclude the movie names for both training and test datasets
...
# adjust.gross is the dependant variable
# "." means include everything except the dependant variable
...
# 1.Country Brazil,Hungray,India,Indonesia,Iran,Netherland,New Zealand, Norway has NA coefficient
# 2.Except Japan and Greece has P-value around 0.2, other coutries have P-value over 0.5
...
# 1. Not enough data for lanaguage levels and movie.years
...
png(paste(image.folder, "plot_MovieMetaProfitPrediction.png", sep = ""))
par(mfrow = c(1,3))
par(mar = c(20, 5, 5, 2))
...
bop01$stats  # show the errors boxplot statstics for both predictions


# visualize the relationship between individual predictors and movie gross profit
png(paste(image.folder, "plot_MovieMetaData_ProfitContributors.png", sep = ""))
par(mfrow=c(2,3))
par(mar = c(4,4,4,2))
...
png(paste(image.folder, "plot_MovieMetaData_NotRealContributors.png", sep = ""))
par(mfrow = c(2,3))
par(mar = c(4,4,4,2))
...
# check out the variables in the dataset of franchise movie detail
names(fran.movie)
...
# store fanchise movie names in a vector
...
# There are serveral hundreds of levels for directors and actors  
# but the franchise movies dataset does not have enough datapoints for so many levels
table(franmovie.regrset$director_name)
table(franmovie.regrset$actor_2_name)
table(franmovie.regrset$actor_1_name)
table(franmovie.regrset$actor_3_name)
...
# Observation:
# 1.Country Brazil,Denmark, Honkong, Taiwan and Thailand has NA coefficient
# 2. Except Canada and USA has P-value less than 0.2, other coutries have P-value no less than 0.4
...
# visualize prediction vs actual data
png(paste(image.folder, "plot_FranchiseMovieProfitPrediction.png", sep = ""))
par(mfrow = c(1,3))
par(mar = c(15, 4, 10, 3))
...
bop02$stats  # show the errors boxplot statstics for both predictions


# visualize the relationship between individual predictors and movie gross profit
png(paste(image.folder, "plot_FranchiseMovie_ProfitContributors.png", sep = ""))
par(mfrow=c(2,3))
par(mar = c(4,4,3,2))
...
png(paste(image.folder, "plot_FranchiseMovie_NotRealContributors.png", sep = ""))
par(mfrow=c(2,3))
par(mar = c(4,4,4,2))
